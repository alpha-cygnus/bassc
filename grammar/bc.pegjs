...
unit
	= name:Id COPEN
		...
		items:(i:unit_item ST_SEP+ { return i; } )*
		CCLOSE { return Unit(name, items, init) }

unit_item = values / layout / chain
...
init = INIT head:init_item tail:(COMMA ids:init_item { return ids; })* { return UnitInit([head].concat(tail)); }
init_item = ids:idr cons:(EQ cons:Cons { return cons; })? { return UnitInitItem(ids, cons || ConsConst(0)); }
